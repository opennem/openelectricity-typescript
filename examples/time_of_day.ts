...
import { DataTable } from "@openelectricity/client/datatable"
...
async function main(): Promise<void> {
  ...
  const endDate = new Date(today.getFullYear(), today.getMonth(), today.getDate()) // midnight today
  ...
  try {
    ...
    const hourlyTable = new DataTable(withHourOfDay, [...datatable.getGroupings(), "hour"], datatable.getMetrics())

    // Group by fuel technology group and hour, calculate mean power
    const byFueltechAndHour = hourlyTable.groupBy(["fueltech_group", "hour"], "mean").sortBy(["fueltech_group", "hour"])

    // Get unique fuel technology groups and sort them
    const fueltechGroups = Array.from(
      new Set(byFueltechAndHour.getRows().map((row) => row.fueltech_group as string))
    ).sort()
    ...
    const hourlyContributions = Array.from({ length: 24 }, (_, hour) => {
      ...
      const hourRows = byFueltechAndHour.filter((row) => row.hour === hour).getRows()

      // Calculate total power for this hour
      const totalPower = hourRows.reduce((sum, row) => sum + ((row.power as number) || 0), 0)

      // Calculate percentages for each fuel technology group
      const hourData = hourRows.reduce(
        (acc, row) => {
          const percentage = totalPower > 0 ? (((row.power as number) || 0) / totalPower) * 100 : 0
      ...
      const totalPercentage = Object.values(hourData).reduce((sum, val) => sum + parseFloat(val), 0)
      if (Math.abs(totalPercentage - 100) > 0.1) {
        console.warn(`Hour ${hour}: Total percentage is ${totalPercentage.toFixed(1)}%`)
      }

      return {
        Hour: `${hour.toString().padStart(2, "0")}:00`,
        ...Object.fromEntries(fueltechGroups.map((group) => [group, hourData[group] ? `${hourData[group]}%` : "0.0%"])),
      }
    })
    ...
    const min = Array.from(totalByHour.entries()).reduce(
      ...
      { hour: 0, power: totalByHour.get(0) || Infinity }
    )

    console.log("\nDemand Patterns:")
    console.log("================")
    console.log(`Peak Demand Hour: ${peak.hour.toString().padStart(2, "0")}:00 (${peak.power.toFixed(0)} MW)`)
    console.log(`Minimum Demand Hour: ${min.hour.toString().padStart(2, "0")}:00 (${min.power.toFixed(0)} MW)`)
  } catch (error) {
